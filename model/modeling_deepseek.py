...
# 使用 Thread-Local 存储，防止多线程/多进程问题（虽然推理时通常是单线程）
_local = threading.local()

def get_expert_load_dict():
    """获取当前线程的专家负载字典，如果不存在则创建并返回。"""
    if not hasattr(_local, "expert_load_dict"):
        _local.expert_load_dict = defaultdict(int)
    return _local.expert_load_dict

def clear_expert_load_dict():
    """清除当前线程的专家负载记录。"""
    if hasattr(_local, "expert_load_dict"):
        _local.expert_load_dict.clear()
print('using local!!!!!!!!!!!!!!!')
import numpy
...
class MoEGate(nn.Module):
    def __init__(self, config):
        ...
        # --- [新增 LASER 参数] ---
        # 假设这些参数可以从 config 中读取或外部设置
        self.laser_enabled = getattr(config, "laser_enabled", False) 
        self.epsilon_high = getattr(config, "laser_epsilon_high", 0.40)
        self.t_fix = getattr(config, "laser_t_fix", 0.80)
        self.candidate_pool_c = getattr(config, "laser_c", self.top_k)
        # --- [结束新增] ---
    ...
    def forward(self, hidden_states):
        ...
        N_tokens = scores.shape[0] # N_tokens = bsz * seq_len
        N_experts = self.n_routed_experts

        
        # --- LASER 路由决策核心 ---
        
        # 默认回退到 vanilla top-k 结果
        topk_weight, topk_idx = torch.topk(scores, k=self.top_k, dim=-1, sorted=False)
        
        # 仅在启用了 LASER 时执行复杂逻辑
        if False: #self.laser_enabled:
            # --- 阶段 1: 计算 Vanilla Top-k 负载作为当前负载估算 ---
            
            # 1. 展平 Vanilla Top-k 索引
            vanilla_flat_idx = topk_idx.view(-1) # Shape: [N_tokens * k]
            
            # 2. 计算每个专家在 Vanilla Top-k 下的负载 (N_e)
            # N_e 是 expert 索引在 vanilla_flat_idx 中出现的次数
            # current_expert_loads 形状: [N_experts]
            current_expert_loads = torch.bincount(vanilla_flat_idx, minlength=N_experts).float()
            
            # 3. 将 PyTorch Tensor 移到 CPU/NumPy 进行循环操作
            scores_cpu = scores.to(torch.float32).cpu().numpy()
            loads_cpu = current_expert_loads.cpu().numpy()
            
            final_topk_idx = []
            final_topk_weight = []
            
            # --- 阶段 2: 逐 Token 应用 LASER 负载感知决策 ---
            
            for i in range(N_tokens):
                s = scores_cpu[i]
                
                # 1. 排序并计算 Top-k Mass (M_k)
                s_indices = numpy.argsort(s)[::-1]
                s_sorted = s[s_indices]
                
                M_k = s_sorted[:self.top_k].sum()

                A_idx = None

                if M_k >= self.epsilon_high:
                    # Case 1: 分数统治性强 -> 沿用 Top-k 路由
                    A_idx = s_indices[:self.top_k]
                    #print("DEBUG: Case 1 - Top-k") # 打印 Case 1
                    
                else:
                    # Case 2: 负载平衡逻辑
                    
                    # 1. 计算扩展阈值 t
                    t = self.t_fix * s_sorted[0]
                    
                    # 2. 初始 T 集合：所有分数 >= t 的专家
                    T_mask = (s >= t)
                    
                    # --- [关键修改]：强制将分数最高的 self.candidate_pool_c 个专家纳入 T ---
                    # 确保 T 至少包含分数最高的 c 个专家，即使他们的分数低于 t
                    T_mask[s_indices[:self.candidate_pool_c]] = True 
                    
                    T_indices = numpy.where(T_mask)[0]
                    
                    # 3. 确定最终候选集 Cand
                    # 由于我们已经强制 T 包含 c 个专家，这里的修剪逻辑可以简化
                    c_star = min(self.candidate_pool_c, len(T_indices))
                    
                    # 确保 Cand_indices 包含我们扩展的 c* 个专家（按分数降序）
                    # 在这里，我们直接使用分数最高的 c* 个专家作为 Cand，因为它们必然包含在 T 中
                    Cand_indices = s_indices[:c_star] # <-- 简化为直接取分数最高的 c* 个专家
                    
                    # 4. 最终分配：选择 k 个负载最轻的
                    # ... (负载感知排序逻辑不变) ...
                    
                    cand_loads = loads_cpu[Cand_indices]
                    cand_scores = s[Cand_indices]
                    
                    sort_key = (cand_loads * 1000.0) - cand_scores # <-- 保持高负载权重
                    
                    final_selection_indices = numpy.argsort(sort_key)[:self.top_k]
                    A_idx = Cand_indices[final_selection_indices]

                    vanilla_idx = s_indices[:self.top_k]
                    # ----------------------------------------------------
                    # 【新增检查代码】
                    
                    # 1. 将两个结果数组转换为集合 (Set)，以忽略顺序
                    laser_set = set(A_idx)
                    vanilla_set = set(vanilla_idx)
                    
                    # 2. 检查集合内容是否不同
                    if laser_set != vanilla_set:
                        # 负载平衡成功：LASER 引入了 Top-k 之外的专家
                        # print(f"SUCCESS: Set changed! Tokens={N_tokens}, TopK Mass={M_k:.4f}")
                        # print(f"  Vanilla Set: {vanilla_set}")
                        # print(f"  LASER Set:   {laser_set}")
                        pass
                        
                        # 可选：打印负载和分数，帮助调试
                        # for idx in vanilla_set.union(laser_set):
                        #     print(f"    Expert {idx}: Load={loads_cpu[idx]:.0f}, Score={s[idx]:.4f}")
                        
                    # ----------------------------------------------------
                
                # 收集结果
                final_topk_idx.append(A_idx)
                final_topk_weight.append(s[A_idx])

            # 将 NumPy 结果转回 PyTorch Tensor
            topk_idx = torch.tensor(numpy.stack(final_topk_idx), device=hidden_states.device)
            topk_weight = torch.tensor(numpy.stack(final_topk_weight), device=hidden_states.device)
...
class DeepseekMoE(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if not self.training:
            # 使用 detach() 确保我们不会将这个张量引入不必要的梯度计算
            self._temp_topk_idx = topk_idx.detach().clone() 
        # --- [结束新增] ---
